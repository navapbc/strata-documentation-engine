...
- Each skill under `skills/` is symlinked into `.claude/skills/<name>` and `.agents/skills/<name>`, so
  cloning the repo loads it without copying: Claude Code reads `.claude/skills`, and Cursor, Codex, and
  Copilot read `.agents/skills`. Edit skills under `skills/`; when you add one, add a matching symlink
  in both dirs.
