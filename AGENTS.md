...
- `.claude/skills` and `.agents/skills` are symlinks to the top-level `skills/`, so cloning the repo
  loads the skills without copying: Claude Code reads `.claude/skills`, and Cursor, Codex, and Copilot
  read `.agents/skills`. Edit skills under `skills/`; the symlinks follow.
