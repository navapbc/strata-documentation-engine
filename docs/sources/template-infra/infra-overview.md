...
designed to be composed with an application template (the Rails app template in v1).
...
- A nondefault **VPC** with public subnets (for the application load balancer), private subnets (for
  the ECS service), and private subnets for the database; plus **NAT gateways** for outbound access
  and **VPC endpoints** for in-VPC access to AWS services.
...
- **CloudWatch Logs / Alarms**, an alarms **SNS topic** to an incident management service, **Cognito**
  for authentication, **SES** for email, and a **Terraform backend** S3 bucket for state.
...
- **Account layer** (`infra/accounts/`) — Terraform backend (S3 state bucket) and the GitHub Actions
  OIDC provider + IAM role/policy. Deployed first because it creates the state bucket the other
  layers' backends use.
- **Network layer** (`infra/networks/`) — the VPC, subnets, VPC endpoints, and (optionally) the WAF
  and Route 53 hosted zones. Shared across applications, environments, and Terraform workspaces.
...
Deployment order follows the dependency graph (`docs/infra/module-architecture.md`): account →
network → build-repository → database → service. See
...
- **Project config** (`infra/project-config/`) — project-wide settings: account/region, resource
  tags, the set of networks, the AWS services GitHub Actions may manage, and system notifications.
...
Day-to-day operations run through Make targets in the root `Makefile`, which call shell scripts in
`bin/`, which in turn call `terraform`. The same scripts back the GitHub Actions workflows
...
selecting the correct `.tfbackend`. You can also call `terraform` directly when needed (e.g.
...
`_exclude` (`README.md`, `LICENSE.md`, `copier.yml`, etc.) are **not** rendered. Separately,
...
- [infra-security-and-access](infra-security-and-access.md) — IAM/OIDC, WAF, HTTPS, and custom domains.
- [infra-capabilities](infra-capabilities.md) — notifications, identity provider, background jobs, monitoring.
