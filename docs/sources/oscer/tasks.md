...
- A `system_process` runs a lambda against the case (here, the two determination services). No task
  record or human action is involved; the service publishes the event that drives the next
  transition.
...
— and renders the SDK's `strata/tasks/index` view. Region scoping is delegated to
