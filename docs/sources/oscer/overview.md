...
requirement, automatically checks externally-sourced hours and income, and — when that is
insufficient — routes the member through self-reported activity reporting with staff review.
...
| Typed `strata_attribute` + attribute types | many models; money, year-month, us-date, name, range, array | [attributes](./attributes.md) |
...
drives it through three automated determination steps — an **external exclusion check**, an
**external exception check**, and an **external community-engagement check** — and, when needed,
human task steps (report activities, staff review). Each terminal step records a `Determination`
against the `Certification` and publishes a domain event; `NotificationsEventListener` (subscribed
to those events) sends the member email.
...
`services/member_oidc_provisioner.rb`), the batch CSV upload pipeline, and the Devise-based user
